...
function logSms(entry) {
  const log = db.read("sms_log");
  log.push({ ...entry, at: new Date().toISOString() });
  db.write("sms_log", log);
}
...
async function sendSMS(toPhone, message) {
  ...
  if (!provider) {
    ...
    logSms({ direction: "out", to: toPhone, message, mode: "mock" });
  ...
  logSms({ direction: "out", to: toPhone, message, mode: "unconfigured-provider" });
